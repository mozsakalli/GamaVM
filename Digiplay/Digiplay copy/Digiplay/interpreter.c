...
extern void vm_compile_method(VM *vm, Method *method);

void interpreter_execute_direct2(VM *vm, Method *method, VAR *args) {
    ...
    op_new:
    {
        Class *cls = resolve_class(vm, method->declaringClass->cp[method->declaringClass->cp[op->index].index1].value.O, 1);
        op->operand.O = (Object*)cls;
        op->handler = handlers[op_new_fast];
        NEXT(0);
    }
    op_new_fast:
        stack[sp++].O = alloc_object(vm, (Class*)op->operand.O);
        NEXT(1);
    op_i2d:
