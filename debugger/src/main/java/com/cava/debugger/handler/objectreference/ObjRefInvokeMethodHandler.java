...
public class ObjRefInvokeMethodHandler extends ObjectReferenceHandler {
    ...
    public int handle(Packet packet, OutBuffer out) {
        ...
        try {
            Object object = VM.getObject(objectId);
            Thread thread = null;//NativeCode.Thread("(JOBJECT)%s", threadId);
            Class cls = (Class)VM.getObject(classId);
            Method method = (Method)VM.getObject(methodId);
        ...
        for (int idx = 0; idx < args.length; idx ++) {
            ...
            switch (tag) {
                ...
                case JdwpConsts.Tag.CLASS_OBJECT:
                    v = VM.getObject(payload.readLong());
                    break;
                default:
